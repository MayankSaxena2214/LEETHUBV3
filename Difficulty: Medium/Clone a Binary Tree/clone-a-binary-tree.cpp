  ...
  map<Node*,Node*>mp;
    /* The function should clone the passed tree and return 
       ...
       Node*getClone(Node*root){
           ...
           mp[root]=cloneRoot;
           cloneRoot->left=getClone(root->left);
           cloneRoot->right=getClone(root->right);
           return cloneRoot;
       }
       void setRandom(Node*root,Node*&cloneRoot){
           if(root==NULL)return;
           cloneRoot->random=mp[root->random];
           setRandom(root->left,cloneRoot->left);
           setRandom(root->right,cloneRoot->right);
           
       }
    Node* cloneTree(Node* tree)
    {
       //Your code here
       Node*clone=getClone(tree);
       setRandom(tree,clone);
       return clone;
    }
};
